...
function DigitalPosition(id, options, streamBundle, instrumentPanel) {
  ...
  class ReactComponent extends React.Component {
    constructor(props) {
      ...
      this.originalLabel = (this.props.options.label && this.props.options.label.replace('navigation.', '')) ||
        props.options.path.replace('navigation.', '');
      this.state = {
    ...
    changePositionFormat(event) {
      ...
      if ((eventOffsetX > 50) || (eventOffsetY > 50)) { return }
      ...
      var newFormatPositionIndex = this.state.formatPositionIndex;
      if (!firstClic) {
      ...
      this.setState({
        formatPositionIndex: newFormatPositionIndex,
        widgetLabel: positionFormats[newFormatPositionIndex]({latitude: this.state.latitude,longitude: this.state.longitude}).label
      });
    }
    ...
    render() {
      ...
      return (
        <svg key={id} onClick={this.changePositionFormat} height="100%" width="100%" viewBox="0 0 20 40" stroke="none">
          ...
          <text x="10" y="4" textAnchor="middle" fontSize="6" dominantBaseline="middle">{this.state.widgetLabel}</text>
        </svg>
      )
    }
  }
  ...
  this.updateUnitData(this, {setStream: false});
}
...
DigitalPosition.prototype.updateStream = function(widget, valueStream) {
