...
import {Bus, combineWith} from 'baconjs';
...
class ComponentClass extends React.Component {
  ...
  valuesToDisplay(values) {
    ...
    if(values) {
      ...
      if(typeof values.angle != 'undefined') {
        newAngle = isNaN(values.angle) ? NaN : radToDeg(values.angle);
  ...
  componentDidMount() {
    ...
    this.unsubscribes.push(
      this.props.optionsBundle.optionsStream.onValue(
        ( ([options,value]) => {
          this.conversion = this.props.optionsBundle.getConversionForOptions();
...
WindMeter.prototype.combineValueStream = function(displayModeIndex) {
  ...
  if(displayModeIndex === displayApparent) {
    ...
    this.options.label = 'Apparent Wind';
  } else if(displayModeIndex === displayTrueGround) {
    paths = [
      'environment.wind.angleTrueGround',
      'environment.wind.speedOverGround'
    ];
    this.options.label = 'True Wind over Ground';
  } else if(displayModeIndex === displayTrueWater) {
    paths = [
      'environment.wind.angleTrueWater',
      'environment.wind.speedTrue'
    ];
    this.options.label = 'True Wind through Water';
  } else {
    this.options.label = '???';
  }
  ...
  if (paths.length == 2) {
    ...
    if(angleSource.stream) {
      angleStream = angleSource.stream;
      this.options.activeSources[angleSource.key] = {
    ...
    if(speedSource.stream) {
      speedStream = speedSource.stream;
      this.options.activeSources[speedSource.key] = {
  ...
  this.unplugs.push(this.optionsStreamBus.plug(
    this.busOptions.withLatestFrom(this.valueStreamBus.toProperty({speed: NaN, angle: NaN}), (options, value) => {
