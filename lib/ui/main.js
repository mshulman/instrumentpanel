...
//import {SvgTouchApp24px} from '../svg/SvgTouchApp24px';
...
// https://github.com/STRML/react-grid-layout/issues/382#issuecomment-299734450
    ...
    return (
      <div>
        ...
        {(!this.state.helpVisible && !this.state.settingsVisible && this.state.widgetFullScreen === false) ?
          <ReactGridLayout
        ...
        {(!this.state.settingsVisible && this.state.widgetFullScreen === false)?ratioShownPath():null}
        {(this.state.widgetFullScreen === false) ?
          <footer style={{textAlign: 'center'}}>
            <hr/>
            <p><a href="https://github.com/SignalK/instrumentpanel">Signal K Instrument Panel</a> version <span id="version">{VERSION}</span></p>
          </footer> : null
        }
      </div>
    );
  ...
  wrapWidget(widget) {
    ...
    return React.createElement(
      'div',
      {
        ...
        onMouseDown: (event) => this.toggleWidgetFullScreen(widget, event)
      },
      widget.reactWidget,
    ...
    if (typeof event !== 'undefined') {
      if (this.state.widgetFullScreen !== false) {
        ...
        event.stopPropagation();
          ...
          var eventOffsetX = event.clientX - rect.left - rect.width / 2;
          var eventOffsetY = event.clientY - rect.top - rect.height / 2;
...
/*
          var halfZoneSize = 50;
          var eventOffsetX = event.nativeEvent.offsetX - event.currentTarget.clientWidth / 2;
          var eventOffsetY = event.nativeEvent.offsetY - event.currentTarget.clientHeight / 2;
          if ((eventOffsetX > -halfZoneSize) && (eventOffsetX < halfZoneSize) &&
            (eventOffsetY > -halfZoneSize) && (eventOffsetY < halfZoneSize)) {
              this.props.model.lens("widgetFullScreen").set(widget);
              event.stopPropagation();
