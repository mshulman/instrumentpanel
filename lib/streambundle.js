...
StreamBundle.prototype.push = function(sourceId, pathValue) {
  ...
  this.allSources.push({
    ...
    path: pathValue.path,
    key: key,
    stream: this.getValueStreamForSourcePath(sourceId, pathValue.path, pathValue.value)
  });
}
...
StreamBundle.prototype.getValueStreamForSourcePath = function(sourceId, path, initialValue) {
  var key = keyForSourceIdPath(sourceId, path);
  ...
  if(!result) {
    ...
    result = this.streams[key] = result.toProperty(initialValue);
  }
  return result;
}
